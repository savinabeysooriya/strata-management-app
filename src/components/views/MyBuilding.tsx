...
const CACHE_KEY = 'mybuildings_cache';
...
export const MyBuilding = () => {
    ...
    const fetchMyBuilding = async (useCache = true) => {
        try {
            ...
            setError('Failed to fetch user assigned building');
            console.error(err);
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div>
            <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
                <Typography variant="h5" gutterBottom sx={{ fontWeight: 'bold' }}>My Buildings</Typography>
                <Button
                    variant="outlined"
